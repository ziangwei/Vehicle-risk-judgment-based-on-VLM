# =============================
# File: infer_bdd_multiframe_h100.py
# Desc: H100-optimized inference with strict frame-index alignment
# =============================
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
from scripts.eval_from_conversations_h100 import check_consistency
import torch
    ...
    if mode=="by_order":
        for k,e in enumerate(obj["evidence"]):
            if isinstance(e, dict):
                e["frame_idx"]= allowed[min(k, len(allowed)-1)]
...
class AtomicJSONLWriter:
    def __init__(self, final_path: str):
        self.final=final_path; self.tmp=final_path+".tmp"; self.fh=None; self.n=0
    def __enter__(self):
        if self.final:
...
def load_model(base_model: str, adapter_path: str, fourbit: bool=False, merge_adapter: bool=False,
    ...
    if adapter_path:
        ...
        if merge_adapter:
            try:
                model = model.merge_and_unload(); print("[Info] LoRA merged & unloaded")
            except Exception:
                pass
    try:
...
def generate_once(model, tokenizer, processor, images: List[Image.Image], prompt_text: str,
    ...
    messages = build_messages(images, prompt_text, allowed_idx=None)  # allowed 已经在 prompt_text 拼接
    ...
    with AtomicJSONLWriter(args.out) as wm, AtomicJSONLWriter(args.json_only_out) as wj:
        for ridx, rec in enumerate(records, 1):
            try:
                ...
                if allowed:
                    allowed = [int(x) - 1 for x in allowed]  # ★ 将 1-based 转成 0-based
                else:
                    allowed = allowed_frame_idxs(image_paths)  # 这个函数内部也已是 0-based 就行
                ...
                if args.append_frame_idx_to_prompt and allowed:
                    prefix = f"本次图片对应的 frame_idx 依次为 {allowed} 。请 evidence[].frame_idx 只能从该列表中选择，并与图片顺序对齐。\n"
                    prompt_text = prefix + prompt_text
                ...
                ok_cons, reason = check_consistency(norm_json)
                if not ok_cons:
                    norm_json = {
                        "risk_level": "low",
                        "hazards": [],
                        "reasoning": ["no_notable_risk"],
                        "metrics": {},
                ...
                wm.write(out_item)
                if args.json_only_out:
                    wj.write(norm_json)
                total+=1; ok+=int(fields_ok)
                print(f"[{ridx}/{len(records)}] fields_ok={fields_ok}")
            except Exception as e:
                wm.write({"error": str(e), "record_index": ridx}); print(f"[Error idx={ridx}] {e}", file=sys.stderr)
